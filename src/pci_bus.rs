...
impl Device for PciBus {
    fn init(&self, dev_manager: &mut DeviceManager) {
        // Allocate port I/O range.
//        dev_manager.register_pio(0xcf8, 8, self.config_address_read, self.config_address_write);
        // Init all devices under the bus.
        for dev in self.devices.iter() {
            dev.lock().expect("failed to acquire the lock").init(dev_manager);
        }
    }
